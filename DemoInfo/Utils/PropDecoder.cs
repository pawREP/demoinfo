...
namespace EHVAG.DemoInfo.Utils
{
    static class PropDecoder
    {
        ...
        internal static int DecodeInt(SendTable.SendProp prop, IBitStream reader)
        {
            if (Util.HasFlagFast(prop.Flags, SendPropertyFlags.VarInt))
            {
                if (Util.HasFlagFast(prop.Flags, SendPropertyFlags.Unsigned))
                {
                    return (int)reader.ReadVarInt();
                }
                else
                {
                    throw new NotImplementedException("Can't decode a ZigZag-Encoded varint. Please open a issue on GitHub and provite this demo");
                }
            }
            else
            {
                if (Util.HasFlagFast(prop.Flags, SendPropertyFlags.Unsigned))
                {
                    return (int)reader.ReadInt(prop.NumBits);
                }
                else
                {
                    return reader.ReadSignedInt(prop.NumBits);
                }
            }
        }

        public static float DecodeFloat(SendTable.SendProp prop, IBitStream reader)
        {
            float fVal = 0.0f;
            ulong dwInterp;
            ...
            dwInterp = reader.ReadInt(prop.NumBits);
        ...
        public static Vector DecodeVector(SendTable.SendProp prop, IBitStream reader)
        {
            if (Util.HasFlagFast(prop.Flags, SendPropertyFlags.Normal)) {
        ...
        {
            int numElements = flattenedProp.Prop.NumElements;
        ...
        public static string DecodeString(SendTable.SendProp prop, IBitStream reader)
        {
            return Encoding.Default.GetString(reader.ReadBytes((int)reader.ReadInt(9)));
        }

        public static Vector DecodeVectorXY(SendTable.SendProp prop, IBitStream reader)
        {
            Vector v = new Vector();
            v.X = DecodeFloat(prop, reader);
            v.Y = DecodeFloat(prop, reader);
        ...
        static bool DecodeSpecialFloat(SendTable.SendProp prop, IBitStream reader, out float result)
        {
            if (Util.HasFlagFast(prop.Flags, SendPropertyFlags.Coord)) {
                result = ReadBitCoord(reader);
                return true;
            } else if (Util.HasFlagFast(prop.Flags, SendPropertyFlags.CoordMp)) {
                result = ReadBitCoordMP(reader, false, false);
                return true;
            } else if (Util.HasFlagFast(prop.Flags, SendPropertyFlags.CoordMpLowPrecision)) {
                result = ReadBitCoordMP(reader, false, true);
                return true;
            } else if (Util.HasFlagFast(prop.Flags, SendPropertyFlags.CoordMpIntegral)) {
                result = ReadBitCoordMP(reader, true, false);
                return true;
            } else if (Util.HasFlagFast(prop.Flags, SendPropertyFlags.NoScale)) {
                result = reader.ReadFloat();
                return true;
            } else if (Util.HasFlagFast(prop.Flags, SendPropertyFlags.Normal)) {
                result = ReadBitNormal(reader);
                return true;
            } else if (Util.HasFlagFast(prop.Flags, SendPropertyFlags.CellCoord)) {
                result = ReadBitCellCoord(reader, prop.NumBits, false, false);
                return true;
            } else if (Util.HasFlagFast(prop.Flags, SendPropertyFlags.CellCoordLowPrecision)) {
                result = ReadBitCellCoord(reader, prop.NumBits, true, false);
                return true;
            } else if (Util.HasFlagFast(prop.Flags, SendPropertyFlags.CellCoordIntegral)) {
                result = ReadBitCellCoord(reader, prop.NumBits, false, true);
                return true;
            }
            result = 0;
